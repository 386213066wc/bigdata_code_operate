...
public class FlinkSQLTumbWinEvent {
    ...
    public static void main(String[] args) {
        ...
        DataStream<TempSensorData> tempSensorData=senv.socketTextStream("bigdata01",9999)
                .map(event -> {
        ...
        Table resultTable = tEnv.sqlQuery("select "+
                "sensorID,"+
                "max(temp),"+
                "TUMBLE_START(evTime,INTERVAL '5' second) as winstart "+
                "from "+table+" GROUP BY sensorID,TUMBLE(evTime,INTERVAL '5' second) ");
